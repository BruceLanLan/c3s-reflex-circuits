...
| `ap.blocked.note` | 这个 agent 正被封锁：它的请求一律被拒。先解除封锁（右边开关面板，或下面的「解除全部封锁」），确认才有意义。 | This agent is blocked: everything it asks is refused. Lift the block first (the switches panel, or "lift every block" below) — a confirm cannot help while it stands. | 这个 agent 正停着，它的每一条都会被拒。要让它继续，先用「恢复全部」解除；只点确认没有用。 | This agent is stopped and everything it asks is refused. To let it continue, lift the stop with "Resume everything" first; a confirm alone does nothing. | 与新的停/恢复用词一致；去掉「右边开关面板」——手机上没有右边 |
| `ap.stop.title` | 全部停下 | Stop everything | 全部停下 | Stop everything | 不改，好词 |
| `ap.stop.sub` | 一下把所有 agent 的 blocked 拉高；要解除得再做一次明确动作 | raises blocked for every agent at once; getting back needs a second, deliberate action | 按住半秒，所有 agent 立刻停下；恢复要另外做一次明确动作 | Hold for half a second and every agent stops at once; resuming is a separate, deliberate action | 配合按住触发（审计 F4）；去掉「拉高 blocked」 |
...
| `ov.none` | 尚未装电路 · 不设限 · 共享 halt 仍对它生效 | no circuit yet · not gated · the shared halt still applies | 还没有规则：这类工具现在**不设限**。全部停下仍然管得到它。 | No rules yet: this kind of tool is **not limited** right now. Stop everything still reaches it. | 「不设限」是最重要的信息，要加重；`ov.none` 不在 `HTML_KEYS`，**若采用加粗需把它加进 `HTML_KEYS`**，否则去掉星号 |
