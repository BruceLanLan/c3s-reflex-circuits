...
* `two_key`：两把钥匙都要在上次放行之后到过，放行花掉两把。后台只有一个操作者令牌，所以它证明的是
  "两次写入"，不是"两个人"。
* `trip_after_refusals` 是唯一**读电路自己判决**的规则：连续 n 次被拒就停机，一次 confirm 复位。
  它值得多写两句，因为 2026-09-17 那天它坏过，而**证明没抓到**。它的安全那一半一直是真的（跳闸
  期间不放行），但复位是死的：跳闸期间每一拍都必然是拒绝，于是带着人那次 confirm 的那一拍又把
  跳闸重新触发了一次，`and not confirm` 那条分支永远轮不到——对着参考实现实测，第 4、6、9 拍各带
  一次 confirm，`refused_tripped` 从头到尾是 1。监控只断言了"跳闸期间不放行"（永远成立、而且平凡
  成立），从没断言"一次 confirm 能解开它"。修法是语义改一个词，**代价是从此多一个监控**
  `violations.refusal_reset`：复位现在是被证明的，不只是被打算的。教训写在这里，因为
  "证明过"从来只覆盖你写下来的那些性质。

**加一条规则要动的地方**，是 `trip_after_refusals` 用两次教出来的：电路仓里是 `Policy` 的字段与
docstring、位宽属性、`layout()`、参考实现、门级构建、`reasons()`，以及 `properties()` 里**每一条你
打算让人相信的性质各一个监控**；后台里是 `_settings`（否则不持久）、`policy_from`（否则装不上）、
`console.py` 开头把拒绝原话映射到位的正则（否则待确认页不知道等哪一位）；页面里是编辑器的输入框和
`domainOf()`。少了后台那两处，规则就是编译好了却没人装得上；少了那个监控，就是它坏了而证明还是绿的。
