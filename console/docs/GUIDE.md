...
**先说清楚一件事。** 后台默认只绑 `127.0.0.1`，只有本机能连。`c3s up --lan` 是你自己选的，也正是
下面这条暴露的来源：配对二维码把操作者令牌放在 URL 的 fragment 里（fragment 不发给服务器，页面
拿到后从地址栏清掉，这一段没问题），但手机上的页面之后**每两秒轮询一次都把令牌放在请求头里明文
发出去**，因为走局域网时不带它什么都读不到；后台是 HTTP，没有 TLS。所以：

* `--lan` 开着的时候，把操作者令牌当作**同一网络上任何人都读得到**的东西。家里的 Wi-Fi 你自己判断，
  咖啡馆不要。
* `c3s pair --no-token` 只是不把令牌放进二维码（扫完手动粘一次），**对轮询没有任何改变**。
* 手机用完就 `c3s token rotate`。这是今天的补救。
* 给手机单独发一个只读令牌正在做，**还没有**（`docs/REDTEAM-2026-09-17.md`，F1）。
...
* **红键只管它认识的名字。** 按下之后才第一次出现的 agent 不在拦下的名单里（页面会写"m 个里拦下了
  n 个"），再按一次会追上。这意味着**一个换名字的 agent 能从红键底下走出去**——红队复现了这一条。
* 堵住换名的开关是 `REFLEX_REQUIRE_AGENT_TOKEN=1`：后台只答应人绑过的名字。但**今天命令行还没有
  绑名字的命令**（后台里有对应的函数，`c3s token` 只有 `rotate`），所以这个模式暂时开不起来。
  默认模式下的绑定是：本机上某个名字第一次带着 `REFLEX_AGENT_TOKEN` 来请求，就绑上了，之后这个名字
  必须一直带同一个令牌，带错的记成 `spoof`。
...
* `two_key`：两把钥匙都要在上次放行之后到过，放行花掉两把。后台只有一个操作者令牌，所以它证明的是
  "两次写入"，不是"两个人"。
* `trip_after_refusals` 是唯一**读电路自己判决**的规则。**它在库里编译并证明过，但后台装不了它**：
  `console.py` 的 `policy_from` 不读这个键，`_settings` 不存它，页面的编辑器也没有它的输入框。装一条
  带它的规则，键会被静默忽略。
...
`REFLEX_REQUIRE_AGENT_TOKEN=1` 关掉 HTTP 建绑定这条路，只答应人绑过的名字——但**绑名字的命令行今天
还没有**（`console.py` 里有对应函数，`c3s_cli/cli.py` 没接），所以这个模式还用不上。
