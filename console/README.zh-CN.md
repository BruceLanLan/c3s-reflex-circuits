...
重点：那次批准被人读过、批过的**那一次**调用花掉了，没有变成一张长期通行证。第 2 拍那条拒绝回复
还带着 `"chain": {"chain_id": 56, "agrees": true, "deployed": false}`——BNB 智能链上的一个公共节点用
同一份 netlist 算了一遍，结论一致。
...
c3s stop-all       # 立刻拒掉一切，包括它从没见过的 agent；--resume 解除
...
* **页面**——七个视图（总览、边界、待确认、任务、流水、Agents、接入），中英双语，默认只在你这台
  机器上。`c3s up --lan` 是自选项。`c3s pair` 会给手机**单独发一个 viewer 令牌**放进二维码，
  在 URL 的 fragment 里（fragment 不会发给服务器），页面拿到后从地址栏清掉。**操作者令牌从此不
  离开这台机器**，这是重点：viewer 令牌能读状态、能批准一个正在等且数字对得上的调用，仅此而已 ——
  装规则、解除封锁、全停、恢复、写心跳，一个都不行，页面会把这些控件禁掉并说明原因；手机的轮询也
  不算心跳。这是今晚才有的，替掉的是一个真漏洞：页面原来每两秒轮询一次都把操作者令牌明文发出去，
  只要页面开着就一直发。`c3s pair --forget <设备 id>` 吊销一台手机；今晚之前配过的手机会收到 403，
  需要重新配对。仍然没有 TLS，所以 viewer 令牌在那个网络上是可读的 —— 它只是不再是打开一切的钥匙。
...
* **红键连它没见过的也一起停。** 它是一个闩锁，后台在问任何电路之前先看它，所以停下期间**一个
  全新的 agent 名字**和**一个没装电路的类**都会被拒 —— 这是实测的，因为它曾经只是一份名字清单，
  换个名字就能走出去。代价是同一枚硬币的反面：停下期间新起的会话，到有人恢复之前是死的，而恢复
  被刻意设计成第二个独立的动作。
